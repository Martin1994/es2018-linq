...
interface TestCase {
    ...
    readonly resultSelector: (x: Owner, y: Iterable<Pet>) => string
}
...
describe("LINQ", () => {
    describe.each<TestCase>([
        {
            ...
            resultSelector: (x, y) => `${x.id}: ${from(y).select(pet => pet.name).toArray().join(", ")}`
        },
        {
    ...
    ])("GroupJoin", ({name, input, output, inner, outerKeySelector, innerKeySelector, resultSelector}) => {
        it(`${name} synchronously`, () => {
            expect(from(input).groupJoin(inner, outerKeySelector, innerKeySelector, resultSelector).toArray()).toEqual(output);
        });

        it(`${name} asynchronously with synchronous inner and synchronous selectors`, async () => {
            expect(await from(input).asAsync().groupJoin(inner, outerKeySelector, innerKeySelector, resultSelector).toArray()).toEqual(output);
        });

        it(`${name} asynchronously with asynchronous inner and synchronous selectors`, async () => {
            expect(await from(input).asAsync().groupJoin(from(inner).asAsync(), outerKeySelector, innerKeySelector, resultSelector).toArray()).toEqual(output);
        });

        it(`${name} asynchronously with synchronous inner and asynchronous selectors`, async () => {
            expect(await from(input).asAsync().groupJoin(
                inner,
